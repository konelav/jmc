...
public:
	...
	BOOL Fire_GuiAction()
	{
        ...
        if ( pT->m_bstrEventsHandlers[ID_GuiAction].Length()  ) {
            ParseScriptlet2((BSTR)(pT->m_bstrEventsHandlers[ID_GuiAction]));
            if ( pT->m_bDropped ) {
                bRet = FALSE;
            }
            return bRet;
		...
		for (nConnectionIndex = 0; nConnectionIndex < nConnections; nConnectionIndex++)
		{
			...
			if (pDispatch != NULL)
			{
                VariantClear(&varResult);
				pvars[0] = pT->m_pvarEventParams[0].bstrVal;
				pvars[1] = pT->m_pvarEventParams[1].bstrVal;
				pvars[2] = pT->m_pvarEventParams[2];
	...
	BOOL Fire_Websocket()
	{
        ...
        if ( pT->m_bstrEventsHandlers[ID_Websocket].Length()  ) {
            ParseScriptlet2((BSTR)(pT->m_bstrEventsHandlers[ID_Websocket]));
            if ( pT->m_bDropped ) {
                bRet = FALSE;
            }
            return bRet;
		...
		for (nConnectionIndex = 0; nConnectionIndex < nConnections; nConnectionIndex++)
		{
			...
			if (pDispatch != NULL)
			{
				...
				pvars[0] = pT->m_pvarEventParams[0].lVal;
				pvars[1] = pT->m_pvarEventParams[1].lVal;
				pvars[2] = pT->m_pvarEventParams[2].bstrVal;
